...
def signin(k,m,frm):
    ...
    if(m=="hoteltaj@123" and k == "Manager"):
        ...
        l1=Label(frame1,text="Welcome!",font=(" Times New Roman",20))
        ...
        uservalue.trace(mode = "w",callback= check_entry1)
        peopleentry = Entry(frame1, textvariable = peoplevalue)
        peoplevalue.trace(mode = "w",callback=check_entry2)
        contactentry = Entry(frame1, textvariable = contactvalue)
        contactvalue.trace(mode = "w",callback=check_entry3)
...
def add_ins():
    ...
    df.to_excel('entries.xlsx',index=False,index_label="ID")
...
def mnu(fr,rt,k):
    ...
    b9 =Button(frame3,text="Exit",command = lambda: to_quit(rt,k),padx=20,pady=10,font="20",bg="grey",fg="white")
    b9.grid(row=20,column=1,pady = 10)
def to_quit(rt,k):
    global df
    global od_lis
    global num_lis
    response = messagebox.askquestion("exit","Do you want to exit restaurant?")
    if response=='yes':
        od_lis.clear()
        num_lis.clear()
        df = df.append({'Name': info.get_name(),'Contact No.': info.get_contactno(),'No. of persons': info.get_no_persons(),'DOB': info.get_date(),'Bill': 0},ignore_index = True)
        df.to_excel('entries.xlsx',index=False,index_label="ID")
        signin(k,"hoteltaj@123",rt)
